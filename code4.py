"""
1) Store the values for each tree in separate variables: tree1, tree2, tree3, tree4, tree5

 2) Add all 5 tree values and store the result in `sum`

 3) Print the total (`sum`) of all 5 trees

 4) Divide `sum` by 5 to find the average and store it in `average`

 5) Print the average (`average`) of all 5 trees
 

 There are five trees in Jack's front yard. He checks each tree to
   find out how tall it is in inches and writes the height on a sheet
   of paper. Jack's list: 98, 94, 41, 96, and 11. What is the average
     height of a tree in Jack's front yard?


"""
tree1= 98
tree2= 94
tree3= 41
tree4= 96
tree5= 11

sum= tree1+tree2+tree3+tree4+tree5
print(sum)

average= sum/5
print(average)
